YUI.add('list-builder', function(Y){
	...
	Y.ListBuilder = Y.Base.create(NAME, Y.Widget, [Y.ListBuilderKeys], {
		...
		_tagCloseButtonClicked: function(e){
			...
			//If the item being deleted is the one which is selected as well, make the current selection as void
			if (currentSelectedTag === tagItemNode){
				self.set(CURRENT_SELECTED_TAG, null);
			}
		...
		deleteSelectedTag: function(currentSelectedTag, selectPrev){
			...
			// if you deleted the very first tag using backspace (i.e. selectPrev === true),
			// then there is no previous tag so you get back to input field
			// if you deleted the last tag using delete (i.e. selectPrev === false), then next tag would be addTagNode,
			//which means you are back to input field
		...
		selectNextTag: function(currentSelectedTag){
			...
			//if currentSelectedTag was the last tag (just before addTagNode) then next would return addTagNode.
			//addTagNode means no currentSelectedTag
