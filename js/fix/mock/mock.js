	    ...
	    if (expectation.method){
	        ...
	        mock[name] = mock[name] ? mock[name] : function() {
	        	...
	        	try {
	                ...
	                for (var i=0, len=currExpec.args.length; i < len; i++){
	            ...
	            if (mock.__expectations.hasOwnProperty(name)){
	                ...
	                if (YLang.isArray(expectation)) {
	                    ...
	                    for(i=0, len=expectation.length; i<len; i++){
	                    	expec = expectation[i];
	                    	Y.Assert.areEqual(expec.callCount, expec.actualCallCount, "Method " + expec.method + "() wasn't called the expected number of times.");
	                    }
	                } else if (expectation.property){
